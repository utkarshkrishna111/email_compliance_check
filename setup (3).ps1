...
# ---------------------------------------------------------------------------
# Helper: given a full path to python.exe, return "major.minor" (e.g. "3.14")
# or $null if the exe is a stub / not a real Python install.
# The Microsoft Store python.exe lives under WindowsApps and silently does
# nothing when called non-interactively, so we explicitly skip that path.
# ---------------------------------------------------------------------------
function Get-PythonVersion([string]$exePath) {
    ...
    try {
        # Use --version which works in every Python 3.x and outputs to stdout.
        # Merge stderr into stdout (2>&1) in case it ever goes to stderr.
...
# ---------------------------------------------------------------------------
# Build a list of candidate exe paths, ordered best-first:
#   1. Every python*.exe found in the real Python install directories
#   2. Whatever PATH resolves for "python", "python3", "py"
# ---------------------------------------------------------------------------
$candidates = [System.Collections.Generic.List[string]]::new()

# Real install directories (newest version folders sort last by name, so reverse)
$installRoots = @(
...
# PATH-resolved names (skip Store stubs automatically via Get-PythonVersion)
foreach ($name in @("python", "python3", "py")) {
    $cmd = Get-Command $name -ErrorAction SilentlyContinue
    if ($cmd -and $cmd.Source -notlike "*WindowsApps*") {
        if (-not $candidates.Contains($cmd.Source)) {
...
# ---------------------------------------------------------------------------
# Walk candidates until one returns a valid version
# ---------------------------------------------------------------------------
$pythonExe = $null
$pyVerRaw  = $null
...
# ---------------------------------------------------------------------------
# If still nothing found, give a clear actionable error
# ---------------------------------------------------------------------------
if (-not $pythonExe) {
    Write-ERR "No working Python installation found."
    Write-Host ""
    Write-Host "        Searched PATH and common install directories." -ForegroundColor DarkGray
    ...
    Write-Host "          3. Or run this script from a normal PowerShell" -ForegroundColor White
    Write-Host "             terminal where 'python --version' works." -ForegroundColor White
    exit 1
}
